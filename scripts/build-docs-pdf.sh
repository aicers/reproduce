...
trap 'rm -f mkdocs.tmp.yml; if [[ "${REPRODUCE_PDF_DEBUG:-0}" != "1" ]]; then rm -rf .pdf-tmp; fi' EXIT

REPRODUCE_LOCALE="$locale" "$python_bin" - <<'PY'
import copy
import os
import sys
...
locale = os.environ.get("REPRODUCE_LOCALE")
if not locale:
    print("REPRODUCE_LOCALE is required", file=sys.stderr)
    sys.exit(1)
...
pdf_plugin = {
    "with-pdf": {
        "enabled_if_env": "REPRODUCE_PDF_EXPORT",
        "output_path": os.path.join(root, "site", "pdf", f"reproduce-manual.{locale}.pdf"),
...
if locale == "ko":
    pdf_plugin["with-pdf"]["cover_title"] = "REproduce"
...
REPRODUCE_PDF_EXPORT=1 "$mkdocs_bin" build -f mkdocs.tmp.yml
