...
| `report_dir`              | Directory where report files are written (required when `report = true`)   | No       | -          |
...
If `report = true` and `report_dir` is missing, REproduce prints:

```text
Configuration error: 'report' is set to true but 'report_dir' is not
configured. Add 'report_dir' pointing to the directory where report
files should be written (absolute or relative path).
Example: report_dir = "/var/lib/reproduce/reports"
```
