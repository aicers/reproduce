...
mod tests {
    ...
    fn byte_accounting_min_max_tracking() {
        ...
        //   else if bytes < min_bytes || min_bytes == 0 { min_bytes = bytes }
        //
        // This means:
        // - First call with 100: 100 > 0 (max_bytes initial) -> max_bytes = 100,
        //   else-if not executed so min_bytes stays 0
        // - Second call with 50: 50 > 100 is false, so check else-if:
        //   50 < 0 is false but min_bytes == 0 is true -> min_bytes = 50
        // - Third call with 200: 200 > 100 -> max_bytes = 200, else-if skipped
        // - Fourth call with 75: 75 > 200 is false, 75 < 50 is false,
        //   min_bytes != 0 -> no update

        report.process(100);
        // First call: 100 > 0 -> max_bytes = 100, else-if skipped
        assert_eq!(
        ...
        assert_eq!(
            report.min_bytes, 0,
            "min_bytes not set on first call due to else-if"
        );

        report.process(50);
        // Second call: 50 > 100 is false, else-if: 50 < 0 is false but min_bytes == 0
        // so min_bytes = 50
        assert_eq!(
            report.min_bytes, 50,
        ...
        report.process(200);
        // Third call: 200 > 100 -> max_bytes = 200, else-if skipped
        ...
        report.process(75);
        // Fourth call: 75 > 200 is false, 75 < 50 is false, min_bytes != 0 -> no update
        assert_eq!(report.min_bytes, 50, "min_bytes should remain 50");
