...
impl TryFromSysmonRecord for FileCreationTimeChanged {
    fn try_from_sysmon_record(rec: &csv::StringRecord, serial: i64) -> Result<(Self, i64)> {
        ...
        let previous_creation_utc_time =
            if let Some(previous_creation_utc_time) = rec.get(field_offset + 7) {
...
impl EventToCsv for ElasticFileCreationTimeChanged {
        ...
        if let Some(hits) = data["hits"]["hits"].as_array() {
            for hit in hits {
                if let Some(message) = hit["_source"]["message"].as_str() {
                    let mut entry = ElasticFileCreationTimeChanged {
                        agent_name: None,
                        agent_id: None,
