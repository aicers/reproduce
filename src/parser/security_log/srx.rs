...
use super::{
    DEFAULT_IPADDR, DEFAULT_PORT, ParseSecurityLog, SecurityLogInfo, SecurityLogParseResult, Srx,
    proto_to_u8,
};
...
fn parse_srx_timestamp_ns(datetime: &str) -> SecurityLogParseResult<i64> {
    Ok(Timestamp::strptime("%Y-%m-%dT%H:%M:%S%.f%:z", datetime)
        .map_err(|e| anyhow!("{e:?}"))?
        .as_nanosecond()
        .try_into()
        .map_err(|e| anyhow!("{e:?}"))?)
...
mod tests {
    ...
    use super::*;
    use crate::parser::security_log::PROTO_TCP;
