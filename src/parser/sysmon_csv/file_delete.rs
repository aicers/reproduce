    ...
    fn try_from_sysmon_record(rec: &csv::StringRecord) -> SysmonCsvResult<(Self, i64)> {
        ...
        let is_executable = if let Some(is_executable) = rec.get(10) {
            if is_executable.eq("true") {
                ...
                return Err(anyhow!("invalid is_executable").into());
            }
        } else {
            return Err(anyhow!("missing is_executable").into());
        };
        let archived = if let Some(archived) = rec.get(11) {
            if archived.eq("true") {
                ...
                return Err(anyhow!("invalid archived").into());
            }
        } else {
            return Err(anyhow!("missing archived").into());
        };
