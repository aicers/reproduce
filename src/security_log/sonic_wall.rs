...
use super::{
    proto_to_u8, timestamp_to_i64, ParseSecurityLog, SecurityLogInfo, SonicWall, DEFAULT_IPADDR,
    DEFAULT_PORT,
};
...
fn parse_sonic_timestamp(datetime: &str) -> Result<Timestamp> {
...
impl ParseSecurityLog for SonicWall {
    fn parse_security_log(
        ...
        let timestamp = parse_sonic_timestamp(datetime).and_then(timestamp_to_i64)? + serial;
