...
use reproduce::parser::sysmon_csv::{ElasticDumpOptions, open_sysmon_csv_file};
...
/// Runs one collector, wraps report accounting, and returns the checkpoint
/// position after the pipeline completes.
macro_rules! run_collector {
    ($collector:expr, $sender:expr, $shutdown:expr, $checkpoint:expr, $report:expr) => {{
        let mut c = $collector;
        $report.start();
        run_pipeline_with_sender($sender, &mut c, $shutdown, &mut |bytes| {
            $report.process(bytes);
        })
        .await?;
        let pos = Collector::position(&c);
...
macro_rules! zeek_or_migration {
    ($iter:expr, $type:ty, $protocol:expr, $migration:expr,
     $skip:expr, $count_sent:expr, $fpm:expr, $dpm:expr, $shutdown:expr,
